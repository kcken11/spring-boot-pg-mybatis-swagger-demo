...
USER_PARAM_START=2
if [ -z "${2}" ] ; then
        echo "No params-file.conf given, use default one "
	USER_PARAM_START=2
	...
	[[ -f "${JAVA_PARAMS_FILE}" ]] && . ${JAVA_PARAMS_FILE}
        ...
        JAVA_PARAMS_FILE=${2}
	USER_PARAM_START=3
        echo "params-file specified is '$JAVA_PARAMS_FILE'"
        [[ -f "${JAVA_PARAMS_FILE}" ]] && . ${JAVA_PARAMS_FILE}
fi

 
if [ -z "${JVM_OPTS}" ] ; then
        JVM_OPTS="-Xms512M -Xmx1024M -Djava.awt.headless=true -XX:MaxPermSize=64M"
fi		
